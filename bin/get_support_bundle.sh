...
do
    ...
    kubectl --namespace=${NAMESPACE} exec ${pod} -- \
        bash -c 'tar -Ocz /var/log/sysdigcloud/ /var/log/cassandra/ /tmp/redis.log /var/log/mysql/error.log /opt/prod.conf 2>/dev/null || true' \
        > ${LOG_DIR}/${pod}/${pod}-support-files.tgz
done
