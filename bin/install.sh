...
create_tls_secret()
{
    ...
    kubectl get secret sysdigcloud-ssl-secret -n ${NAMESPACE} >> $LOG_FILE 2>&1
    if [ $? -ne 0 ]; then
...
create_configmaps()
{
    kubectl get configmap sysdigcloud-config -n ${NAMESPACE}>> $LOG_FILE 2>&1
    if [ $? -ne 0 ]    ; then
...
start_datastores()
{
    kubectl apply -f ${SDC_HOME}/datastores/sdc-mysql-master.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
    kubectl apply -f ${SDC_HOME}/datastores/sdc-redis-master.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
    kubectl apply -f ${SDC_HOME}/datastores/sdc-redis-slaves.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
    kubectl apply -f ${SDC_HOME}/datastores/sdc-cassandra.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
    kubectl apply -f ${SDC_HOME}/datastores/sdc-elasticsearch.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
    kubectl apply -f ${SDC_HOME}/datastores/sdc-mysql-slaves.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
}

start_backend()
{
    kubectl apply  -f ${SDC_HOME}/backend/sdc-api.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
    kubectl apply  -f ${SDC_HOME}/backend/sdc-worker.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
    kubectl apply  -f ${SDC_HOME}/backend/sdc-collector.yaml -n ${NAMESPACE} | tee -a $LOG_FILE
}

start_frontend()
{
    kubectl apply -f ${SDC_HOME}/frontend/ -n ${NAMESPACE} | tee -a $LOG_FILE
}
...
print_service_urls()
{
    ...
    api_url=$(kubectl describe service sdc-api -n ${NAMESPACE} |egrep '^LoadBalancer Ingress:'|awk  '{print $3}') | tee -a $LOG_FILE
    collector_url=$(kubectl describe service sdc-collector -n ${NAMESPACE} |egrep '^LoadBalancer Ingress:'|awk  '{print $3}') | tee -a $LOG_FILE
    printf '\n'
    printf '+%.0s' {1..100}
    printf '\n%.0s' {1..3}
    printf "API URL: %s\n" $api_url
